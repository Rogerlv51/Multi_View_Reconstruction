...
{
    const double f = cam.focal_length;   // 焦距，这里默认xy方向上的焦距相同
    const double *R = cam.rotation;      // 相机的旋转矩阵
    const double *t = cam.translation;   // 相机的平移向量
    const double *X = point.pos;    // 世界坐标系下的三维坐标
    ...
    const double x = xc/zc;   // 归一化平面坐标
    const double y = yc/zc;

    const double r2 = x*x + y*y;
    const double distort = 1.0 + (k0 + k1*r2)*r2;   // 径向畸变函数

    const double u = f* distort*x;   // 重投影坐标x
    const double v = f* distort*y;   // 重投影坐标y
