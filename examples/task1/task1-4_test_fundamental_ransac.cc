...
void calc_fundamental_least_squares(sfm::Correspondences2D2D const & matches, FundamentalMatrix&F){
    ...
    // 填充A矩阵，但实际上这是是用vector来表示，所以需要用i*9+j来表示A(i,j)
...
int main(int argc, char *argv[]){
    ...
    std::ifstream in("/home/rogerlv/ImageBasedModellingEdu/examples/task1/correspondences.txt");
    assert(in.is_open());
